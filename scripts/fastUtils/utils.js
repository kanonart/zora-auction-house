...
async function save() {
  ...
  if (artifactsExist || deploymentsExist) {
    ...
    if (artifactsExist) {
      await copyDirectory(artifactsDirectory, path.join(newArchiveDirectory, 'artifacts'));
    }
    if (deploymentsExist) {
      await copyDirectory(deploymentsDirectory, path.join(newArchiveDirectory, 'deployments'));
    }
  }
}
...
function prepareConfig(networkName, configArray, envVarName, scriptName) {
    let config = configArray ? configArray : parseEnvVariable(envVarName) ? parseEnvVariable(envVarName) : (scriptsConfig[networkName] ? scriptsConfig[networkName][scriptName] : undefined);
    if (!config) {
        throw new Error(`No configuration found for network: ${networkName}`);
    }
    config = !Array.isArray(config) ? scriptsConfig[networkName][scriptName] : config;
    return config;
}
...
function prepareDeployConfig(network,configArray) {
    ...
    let deployConfig = prepareConfig(networkName, configArray, 'DEPLOY_CONFIG', 'deploy.js');
    deployConfig = deployConfig.map(config => typeof config === 'string' ? { name: config } : config);
    let restFlag;
    ...
    if (restFlag) {
        ...
        deployConfig = allContracts.map(config => {
            ...
            if (overrideConfig) {
                config.constructorArgs = overrideConfig.constructorArgs || config.constructorArgs;
    ...
    deployConfig = deployConfig.map(config => {
        if (!config.name) throw new Error('Each element of DEPLOY_CONFIG must have a name property');
        const defaultConfig = scriptsConfig[networkName]['deploy.js'].find(c => c.name === config.name);
        config.libraries = config.libraries || (defaultConfig ? defaultConfig.libraries : {}) || {};
        config.constructorArgs = config.constructorArgs || (defaultConfig ? defaultConfig.constructorArgs : []) || [];
        config.libsSource = config.libsSource || (config.libraries === (defaultConfig ? defaultConfig.libraries : {}) ? 'config' : 'arg');
        config.argsSource = config.argsSource || (config.constructorArgs === (defaultConfig ? defaultConfig.constructorArgs : []) ? 'config' : 'arg');
        return config;
    });
    return deployConfig;
}
...
function prepareInitConfig(network,configArray) {
    let networkName = network === 'hardhat' ? 'localhost' : network;
    let initConfig = prepareConfig(networkName, configArray, 'INITIALIZE_CONFIG', 'initialize.js');
    initConfig = initConfig.map(config => typeof config === 'string' ? { contract: config } : config);
    let restFlag;
    ...
    if (restFlag) {
        ...
        initConfig = allContracts.map(config => {
            const overrideConfig = initConfig.find(c => c.contract === config.contract);
            if (overrideConfig) {
    ...
    const initConfigFinal = initConfig.map(config => {
        if (!config.contract) throw new Error('Each element of INITIALIZE_CONFIG must have a contract property');
        const defaultConfig = scriptsConfig[networkName]['initialize.js'].find(c => c.contract === config.contract);
...
function prepareVerifyConfig(network,configArray) {
    let networkName = network === 'hardhat' ? 'localhost' : network;
    let verifyConfig = prepareConfig(networkName, configArray, 'VERIFY_CONFIG', 'verify.js');
    let restFlag;
