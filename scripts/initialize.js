...
async function main(configArray) {
	...
	const results = await Promise.all(INITIALIZE_CONFIG.map(async (init) => {
		try {
			return await initializeContract(init, signer);
		} catch (e) {
			return { contract: init.contract, address: 'N/A', function: init.function, args: 'N/A', response: {type: 'error', message: e.message} };
		}
	}));
