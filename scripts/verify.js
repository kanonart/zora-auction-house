...
function createConsoleTable() {
  ...
  stream.write([colors.cyan('Contract'), colors.cyan('Address'),colors.cyan( 'Args'), colors.cyan('Libs'), colors.cyan('Response')]);
  return stream;
}
...
function getDeploymentFiles(contractsToVerify, rest, table) {
  ...
  if (contractsToVerify.length > 0) {
    contracts = contracts.filter(contract => {
      ...
      if (!exists) {
        table.write([contractName, '', '', '', colors.red(`${contractName} not found in deployments directory.`)]);
      }
      return exists;
    });
    contracts.sort((a, b) => contractsToVerify.indexOf(a.split('.')[0]) - contractsToVerify.indexOf(b.split('.')[0]));
  }

  if (rest) {
    const allContracts = fs.readdirSync(deploymentsDir);
    const remainingContracts = allContracts.filter(contract => !contracts.includes(contract));
...
function prepareAndDisplayVerificationResultsTable(table, contract, contractAddress, contractArgs, contractLibraries, logMessages, error) {
    if (error) {
        table.write([contract, contractAddress, JSON.stringify(contractArgs), JSON.stringify(contractLibraries), colors.red(error.message)]);
    } else {
        table.write([contract, contractAddress, JSON.stringify(contractArgs), JSON.stringify(contractLibraries), colors.green(JSON.stringify(logMessages))]);
    }
}

async function main(configArray) {
  const [VERIFY_CONFIG, verifyRest] = fastUtils.prepareVerifyConfig(network,configArray);
