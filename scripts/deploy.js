...
async function saveDeploymentData(contractName, contractInstance, args, lib){
	...
	try {
		writeJSONToFile(path.join(__dirname, `../deployments/${contractName}.json`), data);
	} catch (e) {
...
function readAndParseContractArtifact(contractName, dir = path.join(__dirname, '../artifacts/contracts')) {
    const entries = fs.readdirSync(dir, { withFileTypes: true });

    for (let entry of entries) {
        if (entry.isDirectory()) {
            const result = readAndParseContractArtifact(contractName, path.join(dir, entry.name));
            if (result) return result;
        } else if (entry.name === `${contractName}.json` && path.basename(dir) === `${contractName}.sol`) {
            const artifact = JSON.parse(fs.readFileSync(path.join(dir, entry.name), 'utf8'));
            return artifact;
        }
    }
}
...
async function getConstructorArgNames(contractName){
	try {
		...
		let argNames;
		if (constructor) {
			argNames = constructor.inputs.map((input, index) => input.name || `arg${index + 1}`);
		} else {
			argNames = [];
		}
		return argNames;
		...
		console.error(e)
		throw new Error(`Error getting constructor argument names for ${contractName}: ${e}`);
	}
}
...
async function prepareLibraries(libraries) {
    ...
    if (typeof libraries === 'object') {
        const contractsPromises = Object.entries(libraries).map(async ([key, value]) => {
...
async function prepareConstructorArgs(constructorArgs) {
    return await Promise.all(constructorArgs.map(async (arg) => {
...
async function prepareAndDisplayContractsTable(deployConfig) {
    ...
    const contractsDataPromises = deployConfig.map(async (config) => {
        ...
        const argsDisplay = Array.isArray(constructorArg) && constructorArg.length > 0 ? argNames.map((name, index) => `${name}: ${constructorArg[index]}`).join(', ') + colors.grey('\nSource: ') + colors.grey(config.argsSource) : "-";
        const libsDisplay = Object.keys(library).length > 0 ? Object.entries(library).map(([key, value]) => `${key}: ${value}`).join(', ') + colors.grey('\nSource: ') + colors.grey(config.libsSource) : "-";
...
async function deployContract(contractName, libraries, constructorArgs) {
    ...
    const Contract = await hre.ethers.getContractFactory(contractName, {libraries: lib});
...
async function main(configArray) {
    try {
        ...
        writeJSONToFile(path.join(__dirname, `../deployments/build-info.json`), {buildTime: new Date().toISOString(), network: hre.network.name, signer: signer.address, config: DEPLOY_CONFIG});
        ...
        for (let i = 0; i < DEPLOY_CONFIG.length; i++) {
            ...
            try {
                const contractInstance = await deployContract(contract, DEPLOY_CONFIG[i].libraries, DEPLOY_CONFIG[i].constructorArgs);
                createAndDisplayRowTable(stream, contract, contractInstance.address, colors.green('tx: ',contractInstance.deployTransaction.hash));
                deployedContracts[contract] = contractInstance;
            } catch (e) {
                createAndDisplayRowTable(stream, contract, '', colors.red(e.message.substring(0, 1000)));
                process.exit(1);
            }
        }
